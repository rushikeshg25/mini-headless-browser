...
export function paint(root: Box): string {
  const width = Math.max(1, root.x + root.width);
  const height = Math.max(1, root.y + root.height);
  const grid: Grid = Array.from({ length: height }, () => Array(width).fill(' '));

  const draw = (box: Box): void => {
    if (box.type === 'text') {
...
function putText(grid: Grid, x: number, y: number, text: string, maxWidth: number): void {
  const clipped = text.slice(0, maxWidth);
  for (let i = 0; i < clipped.length; i++) set(grid, x + i, y, clipped[i]);
}

function drawBorder(grid: Grid, x: number, y: number, w: number, h: number): void {
  if (w < 2 || h < 2) return;
  const right = x + w - 1;
  const bottom = y + h - 1;
